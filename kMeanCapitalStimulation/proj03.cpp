...
main( int argc, char *argv[ ] )
{
	...
	for( int n = 0;  n < MAXITERATIONS; n++ )
	{
		...
		for( int k = 0; k < NUMCAPITALS; k++ )
		{
	Capitals[k].longitude = Capitals[k].numsum > 0 ? Capitals[k].longsum / Capitals[k].numsum : Capitals[k].longitude;
	Capitals[k].latitude = Capitals[k].numsum > 0 ? Capitals[k].latsum / Capitals[k].numsum : Capitals[k].latitude;
